...
@section('content')
    <h2 class="font-pixel text-sm text-retro-accent2 mb-6">{{ __('companies.edit') }}</h2>

    <form action="{{ route('admin.companies.update', $company) }}" method="POST" enctype="multipart/form-data" class="pixel-border bg-retro-panel p-6 max-w-md" novalidate>
        @csrf
        @method('PUT')

        @if($company->image)
            <div class="mb-4">
                <img src="{{ $company->image }}" class="w-20 h-20 object-contain bg-retro-panelLight pixel-border !border-2">
            </div>
        @endif
        ...
        <div class="mb-6">
            ...
            <p class="font-mono text-sm text-retro-muted mt-1">{{ __('companies.fields.image_hint') }}</p>
            @error('image')
                <p class="text-red-400 font-mono text-sm mt-1">{{ $message }}</p>
            @enderror
        </div>
