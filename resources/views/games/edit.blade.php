...
@section('content')
    <div class="container mx-auto my-8 p-6 bg-gray-800 shadow-md rounded-lg">
        <h1 class="mb-4 text-center text-xl font-bold text-white">Edit Game</h1>

        <form action="{{ route('games.update', $game->id) }}" method="POST" enctype="multipart/form-data" class="bg-gray-700 p-4 rounded">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="publisher_name" id="publisher_name" value="{{ old('publisher_name', $game->publisher_name) }}" required
                       class="mt-1 block w-full p-2 rounded-md border border-gray-600 bg-gray-800 text-white" />
            </div>
            ...
            <div class="mb-4">
                ...
                <input type="text" name="developer_name" id="developer_name" value="{{ old('developer_name', $game->developer_name) }}" required
                       class="mt-1 block w-full p-2 rounded-md border border-gray-600 bg-gray-800 text-white" />
            </div>
                    ...
                    @foreach($tags as $tag)
                        <div class="flex items-center">
                            <input type="checkbox" name="tags[]" value="{{ $tag->id }}" id="tag-{{ $tag->id }}"
                                   ...
                                   onclick="toggleTag(this, '{{ $tag->color }}')">
                            <label for="tag-{{ $tag->id }}"
                                   class="flex items-center justify-center h-10 px-4 rounded-md border border-gray-600 text-gray-200
                                   transition-colors duration-200 cursor-pointer {{ in_array($tag->id, $game->tags->pluck('id')->toArray()) ? $tag->color : 'bg-gray-800' }}">
                                {{ ucfirst($tag->name) }}
                            </label>
                        </div>
                    @endforeach
            ...
            <button type="submit"
                    class="mt-3 w-full bg-blue-500 hover:bg-blue-600 text-white font-bold py-2 px-4 rounded">Update Game</button>
        </form>
    </div>
